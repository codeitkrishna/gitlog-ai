...
export default function TagSelector({
  ...
  if (!loading && tags.length === 0) {
    return (
      <div className="px-3 py-2 bg-[#0d1317] border border-[#1e3a2a] rounded-lg text-xs text-[#7fc28e]">
  ...
  return (
    <div className="space-y-3">
      {/* From Tag */}
      <div>
        <label className="block text-xs text-[#7fc28e] mb-1.5">From tag</label>
        <select
          value={fromTag || ''}
          onChange={(e) => setFromTag(e.target.value || null)}
          disabled={loading}
          className="w-full px-3 py-2 bg-[#0d1317] border border-[#1e3a2a] rounded-lg text-sm text-[#CAFFD6] focus:outline-none focus:border-[#22c55e] disabled:opacity-50 cursor-pointer"
        >
      ...
      {/* To Tag */}
      <div>
        <label className="block text-xs text-[#7fc28e] mb-1.5">To tag</label>
        <select
          value={toTag || ''}
      ...
      <div className="px-3 py-2 bg-[#0d1317] border border-[#1e3a2a] rounded-lg text-sm">
        <div className="flex items-center gap-2 text-[#CAFFD6]">
          ...
          {fromTag && toTag ? (
            ...
            <span className="text-[#7fc28e]">No tag range selected</span>
          )}
        </div>
      </div>
